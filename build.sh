...
if [[ ! -z ${BUILD_KERNEL_BRANCH} ]]; then
	...
	if [[ ${BUILD_KERNEL_BRANCH} == "mainline" ]]; then
		...
		LOCALVERSION=" - KernelSU ${GITHUB_RUN_NUMBER}"
		export LOCALVERSION=" - KernelSU ${GITHUB_RUN_NUMBER}"

		if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
			FILE_OUTPUT=KernelSU-${GITHUB_RUN_NUMBER}.A${BUILD_ANDROID_PLATFORM}.${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${FILE_NAME_SELINUX}_${BUILD_DEVICE_NAME^}.CI.zip
		else
			FILE_OUTPUT=KernelSU-${GITHUB_RUN_NUMBER}.A${BUILD_ANDROID_PLATFORM}.${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${FILE_NAME_SELINUX}-NoRoot_${BUILD_DEVICE_NAME^}.CI.zip
		fi
	fi
else
	if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
